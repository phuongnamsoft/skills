...
Events include when messages are [published](https://ably.com/docs/pub-sub.md#publish), when [presence](https://ably.com/docs/presence-occupancy/presence.md#trigger-events) events occur, changes in channel occupancy and when channels are created or discarded. Data can be delivered individually or in batches to external services.

![Ably Webhooks Overview](https://raw.githubusercontent.com/ably/docs/main/src/images/content/diagrams/webhooks-overview.png)
...
Use `channel.lifecycle` and `channel.occupancy` sources instead of a `channel.message` source on [metachannels](https://ably.com/docs/metadata-stats/metadata/subscribe.md#metachannels)
</Aside>
...
Multiple requests can be in-flight at once, however be aware there is a [limit on concurrency](https://ably.com/docs/platform/pricing/limits.md#integrations). If it is exceeded then new messages are placed in a short 10 message queue. If that is exceeded then further messages are rejected.
...
The Ably SDK automatically decodes messages sent over the Realtime service into [`message`](https://ably.com/docs/api/realtime-sdk/types.md#message) objects. However, batched, enveloped webhook payloads require explicit decoding using:

* [`Message.fromEncoded`](https://ably.com/docs/api/realtime-sdk/messages.md#message-from-encoded) for an array of messages.
* [`Message`](https://ably.com/docs/api/realtime-sdk/types.md#message) for a single message.

The benefits of decoding include fully restoring `data` to its original datatype using encoding. Additionally, it supports automatic decryption when an [encryption](https://ably.com/docs/channels/options/encryption.md) key is provided. Its recommended to decode all messages received via webhooks to ensure proper data handling.
...
Presence messages sent [over the realtime service](https://ably.com/docs/channels.md) are automatically decoded into [`PresenceMessage`](https://ably.com/docs/api/realtime-sdk/types.md#presence-message) objects by the Ably client library. For webhooks, you need to do this manually using [`PresenceMessage.fromEncodedArray`](https://ably.com/docs/api/realtime-sdk/presence.md#presence-from-encoded-array) on the `data.presence` array, or [`PresenceMessage.fromEncoded`](https://ably.com/docs/api/realtime-sdk/presence.md#presence-from-encoded) on an individual entry. These methods convert the encoded values into [`PresenceMessage`](https://ably.com/docs/api/realtime-sdk/types.md#presence-message) objects—either as an array or a single instance.

This allows you to decode the numerical action into a [`Presence action`](https://ably.com/docs/api/realtime-sdk/presence.md#presence-action) string (such as `enter`, `update`, or `leave`), fully decode the `data` (using the `encoding`) back into its original datatype or an equivalent in the client library, and, if you're using [encryption](https://ably.com/docs/channels/options/encryption.md), pass your encryption key to decrypt the `data`.
...
| `data.status` | A [`ChannelStatus`](https://ably.com/docs/api/realtime-sdk/channel-metadata.md#channel-details) object that describes the channel's current state. |
...
Webhook [presence](https://ably.com/docs/presence-occupancy/presence.md) events contain raw `presence` data in the `presence` array.
...
The payload will contain the [data](https://ably.com/docs/api/realtime-sdk/presence.md#presence-message) of the `Presence` message.

The following example demonstrates a non-enveloped [enter](https://ably.com/docs/api/realtime-sdk/presence.md#enter) presence event:
